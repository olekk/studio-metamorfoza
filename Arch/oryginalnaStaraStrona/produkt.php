...
if ($Produkt->CzyJestProdukt == true) {
    ...
    $pola = array(array('products_viewed', $Produkt->info['ilosc_wyswietlen'] + 1));		
    ...
    if ( KARTA_PRODUKTU_KOSZTY_WYSYLKI == 'tak' ) {
        ...
        foreach ( $tablicaWysylek as $Wysylka ) {
            ...
            if ( $Produkt->info['darmowa_wysylka'] == 'tak' ) {
                 $Wysylka['wartosc'] = 0;
            }
            //
            ...
            //
            $CenaWysylki = $GLOBALS['waluty']->FormatujCene($Wysylka['wartosc'], 0, 0);
    ...
    // wyglad srodkowy
    $srodek = new Szablony($Wyglad->TrescLokalna($WywolanyPlik), $Produkt, $NajtanszaWysylka);   
    ...
    if ( KARTA_PRODUKTU_KOSZTY_WYSYLKI == 'tak' ) {
        ...
        if ( is_array($NajtanszaWysylka) ) {
            //
            $CenaWysylki = $GLOBALS['waluty']->FormatujCene($NajtanszaWysylka['koszt'], 0, 0);
    ...
    if ( INTEGRACJA_FB_OPINIE_WLACZONY == 'tak' ) {
        //
        $KomentarzeFb = '<div id="fb-root"></div><script>(function(d, s, id) { var js, fjs = d.getElementsByTagName(s)[0]; if (d.getElementById(id)) return; js = d.createElement(s); js.id = id; js.src = "http://connect.facebook.net/pl_PL/all.js#xfbml=1"; fjs.parentNode.insertBefore(js, fjs); }(document, \'script\', \'facebook-jssdk\'));</script>';
    ...
    foreach ( $Produkt->dodatkowePolaTekstowe as $TxtPole ) {
        ...
        //
        switch( $TxtPole['typ'] ) {
            case 'input': $PolaTekstowe .= '<input type="text" id="pole_txt_' . $TxtPole['id_pola'] . '" name="pole_txt_' . $TxtPole['id_pola'] . '" class="UsunTekst" value="' . $TxtPole['domyslny'] . '" data-text="" size="30" />'; break;
