...
if ($prot->wyswietlStrone) {
    ...
    if (isset($_POST['akcja']) && $_POST['akcja'] == 'zapisz_duplikuj') {
        ...
        $sql = $db->insert_query('products' , $pola);
        ...
        if (!isset($_POST['id_kat'])) {
            ...
            $pola = array(
                    array('products_id',$id_dodanej_pozycji),
                    array('categories_id',$infoKat['categories_id']));        
        ...
        $sql = $db->update_query('products_attributes', $pola, "products_id = '".$filtr->process($_POST['id_unikalne'])."'");
        if (CECHY_MAGAZYN == 'tak') {
            $sql = $db->update_query('products_stock', $pola, "products_id = '".$filtr->process($_POST['id_unikalne'])."'");
        }
        unset($pola);
        ...
        if (CECHY_MAGAZYN == 'tak') {
            //
            $ogolna_ilosc = 0;
            $zapytanie_pola = "select distinct * from products_stock where products_id = '" . $id_dodanej_pozycji . "'";
            $sqls = $db->open_query($zapytanie_pola);   
            //
            if ((int)$db->ile_rekordow($sqls) > 0) {
                ...
                $sql = $db->update_query('products', $pola, "products_id = '".$id_dodanej_pozycji."'");
                //
            }
        ...
        if (isset($id_dodanej_pozycji) && $id_dodanej_pozycji > 0) {
            ...
            unset($tablica_kat);
            //
            Funkcje::PrzekierowanieURL('produkty.php?id_poz='.$id_dodanej_pozycji . $kat);
        } else {
    ...
    <div id="cont">
        ...
        <form action="produkty/produkty_duplikuj.php" method="post" id="poForm" class="cmxform" onsubmit="return sprKat()"> 

        <div class="poleForm">
            <div class="naglowek">Duplikowanie produktu</div>
            ...
            if ((int)$db->ile_rekordow($sql) > 0) {
                ...
                <table style="width:100%"><tr>
                    ...
                    <td id="lewe_zakladki" style="vertical-align:top">
                        ...
                        <span onclick="gold_tabs_horiz('20','<?php echo $tab_20 = rand(0,999999); ?>')" class="a_href_info_zakl" id="zakl_link_20">Sprzedaż elektroniczna</span>
                        <span onclick="gold_tabs_horiz('16','<?php echo $tab_16 = rand(0,999999); ?>')" class="a_href_info_zakl" id="zakl_link_16">Filmy YouTube</span>
                        <span onclick="gold_tabs_horiz('17','<?php echo $tab_17 = rand(0,999999); ?>')" class="a_href_info_zakl" id="zakl_link_17">Filmy FLV</span>
