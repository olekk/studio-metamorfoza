    ...
    if (isset($_GET['parametr'])) {
        ...
        if ($ile_pozycji > 0) {
            ...
            while ($info = $sql->fetch_assoc()) {
                  ...
                  $tablica[] = array(NUMER_FAKTURY_PREFIX . str_pad($info['invoices_nr'], FAKTURA_NUMER_ZERA_WIODACE, 0, STR_PAD_LEFT) . strftime(NUMER_FAKTURY_SUFFIX, strtotime($info['invoices_date_generated'])),'center');
                  ...
                  if ( trim($info['invoices_billing_company_name']) != '' ) {
                  ...
                  if ( trim($info['invoices_billing_company_name']) != '' ) {
                    if ( trim($info['invoices_billing_nip']) != '' ) {
                        $wyswietlana_nazwa .= 'NIP: ' . $info['invoices_billing_nip'];
                    }   
                   } else {
                    if ( trim($info['invoices_billing_pesel']) != '' ) {
                        $wyswietlana_nazwa .= 'PESEL: ' . $info['invoices_billing_pesel'];
                    }   
                  }

                  $tablica[] = array(date('d-m-Y',strtotime($info['invoices_date_generated'])),'center');
                  $tablica[] = array(date('d-m-Y',strtotime($info['invoices_date_payment'])),'center');

                  $tablica[] = array($wyswietlana_nazwa,'left');

                  if ( $info['invoices_payment_status'] == '0' ) {
                      $status .= '<img src="obrazki/aktywny_off.png" alt="Fatura nieopłacona" title="Fatura nieopłacona" />';
                  } else {
                      $status .= '<img src="obrazki/aktywny_on.png" alt="Fatura opłacona" title="Fatura opłacona" />';
                  }
                  ...
                  $tekst .= '<a href="sprzedaz/zamowienia_faktura_pdf.php'.$zmienne_do_przekazania.'&amp;id='.$info['invoices_id'].'"><img src="obrazki/pdf.png" alt="Wydrukuj fakturę VAT" title="Wydrukuj fakturę VAT" /></a>';
                  ...
                  $tekst .= '<a href="sprzedaz/zamowienia_faktura_usun.php?id_poz='.(int)$info['invoices_id'].'"><img src="obrazki/kasuj.png" alt="Skasuj" title="Skasuj" /></a>';
        ...
        <div id="caly_listing">
            ...
            <div id="wyszukaj">
                ...
                if ( Listing::wylaczFiltr(basename($_SERVER['SCRIPT_NAME'])) == true ) {
                  echo '<div id="wyszukaj_ikona"><a href="sprzedaz/zamowienia_faktury.php?filtr=nie"><img src="obrazki/reset_szukaj.png" alt="Anuluj wyszukiwanie" title="Anuluj wyszukiwanie" /></a></div>';
                }
                ?>                 
